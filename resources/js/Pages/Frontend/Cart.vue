    ...
    <FrontendLayout :itemsCartCount="products.length">
        <div class="bg-white">
            <div class="mx-auto max-w-2xl px-4 pb-24 pt-16 sm:px-6 lg:max-w-7xl lg:px-8">
                ...
                <form @submit.prevent="route('order')"
                        ...
                        <ul role="list" class="divide-y divide-gray-200 border-b border-t border-gray-200">
                            <li v-for="(product, productIdx) in products" :key="product.id" class="flex py-6 sm:py-10">
                                <div class="flex-shrink-0 w-24">
                                    <img :src="product.media[0]?.original_url" :alt="product.title" width="200"
                                ...
                                <div class="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                                    <div class="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
                                        ...
                                        <div class="mt-4 sm:mt-0 sm:pr-9">
                                            ...
                                            <!-- @change="updateQuantity(product.product.slug, product.quantity)" -->
                    ...
                    <section aria-labelledby="summary-heading"
                        ...
                        <dl class="mt-6 space-y-4">
                            <div class="flex items-center justify-between">
                                ...
                                <dd class="text-sm font-medium text-gray-900">${{ total }} x {{ form.quantity }}</dd>
                            </div>
                            ...
                            <div class="flex items-center justify-between border-t border-gray-200 pt-4">
                                ...
                                <dd class="text-base font-medium text-gray-900">${{ total * form.quantity }}</dd>
                            </div>
                        </dl>
...
const updateQuantity = (slug, quantity) => {
    router.put(route('cart.update-quantity', { product: slug }), {
        quantity: quantity,
    });
}
const removeItem = (slug, index) => {
    router.post(route('cart.remove'), { product: slug, index });
}
